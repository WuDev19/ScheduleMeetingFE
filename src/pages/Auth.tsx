...
import { KeyRound, Mail, User as UserIcon, Phone, UserCheck } from 'lucide-react';
      ...
      >
        ...
        {activeTab === 'register' && (
          <form onSubmit={handleSignUpSubmit(onSignUp)} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxHeight: '450px', overflowY: 'auto', paddingRight: '0.25rem' }}>
            ...
            <div className="form-group">
              <label className="form-label" style={{ color: '#cbd5e1' }} htmlFor="signup-role">Vai trò của bạn</label>
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-tertiary)' }}>
                  <UserCheck size={16} />
                </span>
                <select
                  id="signup-role"
                  className="form-control"
                  style={{ width: '100%', paddingLeft: '2.5rem', backgroundColor: 'rgba(30, 41, 59, 0.8)', borderColor: 'rgba(255,255,255,0.1)', color: '#ffffff', appearance: 'none' }}
                  {...signUpRegister('role')}
                >
                  <option value="REGISTER" style={{ color: 'var(--text-primary)' }}>Nhân viên (REGISTER)</option>
                  <option value="APPROVER" style={{ color: 'var(--text-primary)' }}>Người duyệt lịch (APPROVER)</option>
                  <option value="ADMIN" style={{ color: 'var(--text-primary)' }}>Quản trị viên (ADMIN)</option>
                </select>
              </div>
              {signUpErrors.role && <span className="form-error">{signUpErrors.role.message}</span>}
            </div>
